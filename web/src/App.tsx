    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route element={<AppLayout />}>
          {/* The one canvas route. CanvasRouter dispatches by chain
              family: the EVM canvas (5 mainnets) or the Solana devnet
              canvas, selected by the top-bar chain picker (?chain=
              solana). Both share this layout's chrome. */}
          <Route path="/" element={<CanvasRouter />} />
          <Route path="/pixel/:coord" element={<CanvasRouter />} />
          {/* Legacy /stats links still resolve to the canvas page; the
              stats content now lives inline below the canvas. */}
          <Route path="/stats" element={<CanvasRouter />} />
          ...
          {/* Back-compat: the Solana canvas used to live at its own
              route. It's now a chain in the main picker, so redirect
              the old URL to the unified canvas with Solana selected. */}
          <Route path="/solana" element={<Navigate to="/?chain=solana" replace />} />
