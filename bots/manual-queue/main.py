...
Same chain-scan / notability filter as the original twitter-paints bot,
but instead of posting to X (which moved to a paid pay-per-use model in
Feb 2026 and would cost ~$0.20 per tweet because every Tagwall tweet
includes a deep-link URL), this script appends formatted, ready-to-paste
tweet text to QUEUE.md in this repo.

Operator workflow:
  1. Cron runs every 30 min, prepends new notable paints to QUEUE.md.
  2. Operator views QUEUE.md on github.com (desktop or mobile).
  3. For each entry: copy the tweet text from the fenced block, post
     manually from the @tagwall_io_bot account, then tick the checkbox
     (auto-commits via the GitHub web UI) or delete the entry.

Run on a 30-minute GitHub Actions cron. Reads state.json for the last
block processed per chain, queries each chain for new Painted events,
filters to notable paints, prepends them to QUEUE.md, advances the
marker.
...
Env (all required unless noted):
  ...
  MANUAL_QUEUE_MIN_PIXELS     — defaults to 100
  MANUAL_QUEUE_MAX_PER_RUN    — defaults to 50 (safety cap on first-run floods)
  DRY_RUN                     — '1' / 'true' to skip writing QUEUE.md
"""
import json
import os
import sys
...
The queue is generated every 30 min by `.github/workflows/manual-queue.yml`
scanning all four EVM chains for `Painted` events at or above
`MANUAL_QUEUE_MIN_PIXELS` pixels.
