...
notable paints and weekly recap copy on `tagwall.io/tweets` and in a
Markdown queue for the operator to post by hand from `@tagwall_io_bot`.

Was called `manual-queue` until the May 2026 launch — the name reflected
the only manual step (posting to X). Renamed once everything else (cron,
summaries, auto-publish, frontend page) was wired up; describing the
whole pipeline as "manual" became misleading.

## How it works

- `.github/workflows/tweets.yml` — cron every 30 min + manual dispatch.
- `main.py` — single-shot script. Reads `state.json` for the last block
  ...
  filters to "notable" paints (≥ `TWEETS_MIN_PIXELS` pixels), formats
  each as a ready-to-paste tweet, prepends them to **both** `QUEUE.md`
  (Markdown for git-side review) and `web/public/queue.json` (consumed
  by the `/tweets` page on tagwall.io). Also computes a trailing-7-day
  per-chain summary into `web/public/summary.json`.
...
- `../../web/public/summary.json` — 7-day aggregates rendered as the
  per-chain summary cards and the cross-chain recap tweet template at
  the top of the page.
...
1. Open the page in any browser. Cross-chain recap at top, per-chain
   weekly cards below, newest per-paint entries at the bottom.
2. For each entry: click **Copy tweet**, optionally edit, post manually
   from `@tagwall_io_bot` on X, then (for per-paint entries) click
   **Mark posted** to dim and collapse it. Posted state is per-browser
   localStorage.

**Git (when you want a permanent paper trail):**
`bots/tweets/QUEUE.md` on the repo web UI.
...
| `TWEETS_MIN_PIXELS` | optional, defaults to `100`. Legacy alias `MANUAL_QUEUE_MIN_PIXELS` is still read if set. |
| `TWEETS_MAX_PER_RUN` | optional, defaults to `50`. Legacy alias `MANUAL_QUEUE_MAX_PER_RUN` is still read if set. |
...
Trigger manually from GitHub Actions → "Tweets bot" → **Run
...
`TWEETS_MIN_PIXELS=100` (default) is the original Twitter-bot
threshold. Raise to 500 or 1,000 if the queue is too noisy. Single-
pixel reservation paints are always skipped.

`TWEETS_MAX_PER_RUN=50` (default) caps a runaway first-run flood.
...
3. Disable the cron on `tweets.yml` (or delete it).
